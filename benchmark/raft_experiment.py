...
def sh(cmd: List[str], *, cwd: Path = ROOT, check: bool = True, capture: bool = True) -> str:
    ...
    if check and proc.returncode != 0:
        raise RuntimeError(
            "command failed (%s):\nstdout:\n%s\nstderr:\n%s"
            % (" ".join(cmd), proc.stdout or "", proc.stderr or "")
        )
    return proc.stdout if capture else ""


def load_ok_rows(path: Path) -> Dict[str, List[int]]:
...
def summarize_csv(name: str, rows: Dict[str, List[int]]) -> Dict[str, float]:
...
def write_comparison(baseline_csv: Path, xdp_csv: Path, out_csv: Path) -> None:
    baseline = summarize_csv("baseline", load_ok_rows(baseline_csv))
    xdp = summarize_csv("xdp", load_ok_rows(xdp_csv))
    baseline_summary = load_summary_metrics(summary_path_for(baseline_csv))
    xdp_summary = load_summary_metrics(summary_path_for(xdp_csv))
    speedup = (
...
def compile_and_load_ebpf(cluster: Dict[str, object], cfg: ExperimentConfig) -> None:
    ...
    if cfg.enable_leader_quorum or cfg.enable_tc_broadcast:
        multipass("node1", "cd ~/electrode-lab/xdp && make clean && make leader")
